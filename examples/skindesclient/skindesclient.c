...
{
  string name = "skindesclient";
  map< int, string > menus;
  menus.insert(pair<int, string>(meListMain, "menulistmain.xml"));
  menus.insert(pair<int, string>(meListSub, "menulistsub.xml"));
  menus.insert(pair<int, string>(meDetail, "menudetail.xml"));
  if (!skindesignerservice::ISkindesignerService::CallRegisterPlugin(name, menus)) {
